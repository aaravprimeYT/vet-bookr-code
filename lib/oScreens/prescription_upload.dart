...
class PrescriptionUpload extends StatefulWidget {
  PrescriptionUpload({Key? key}) : super(key: key);
...
class _PrescriptionUploadState extends State<PrescriptionUpload> {
  ...
  Future<void> uploadPrescriptionToFirebase(XFile image) async {
    try {
      final imageRef = storageRef.child(
          "Users/Prescriptions/$petId/${image.name}");
            ...
            ElevatedButton(
              ...
              onPressed: () {
                for (var i = 0; i < 6; i++) {
                  ...
                  if (i == 0 && image1 != null) {
                    var prescription = Prescription("${controller1.text}", "${image1?.path}", i);
                    prescArray.add(prescription);
                  }
                  if (i == 1 && image2 != null) {
                    var prescription = Prescription("${controller2.text}", "${image2?.path}", i);
                    prescArray.add(prescription);
                  }
                  if (i == 2 && image3 != null) {
                    var prescription = Prescription("${controller3.text}", "${image3?.path}", i);
                    prescArray.add(prescription);
                  }
                  if (i == 3 && image4 != null) {
                    var prescription = Prescription("${controller4.text}", "${image4?.path}", i);
                    prescArray.add(prescription);
                  }
                  if (i == 4 && image5 != null) {
                    var prescription = Prescription("${controller5.text}", "${image5?.path}", i);
                    prescArray.add(prescription);
                  }
                  if (i == 5 && image6 != null) {
                    var prescription = Prescription("${controller6.text}", "${image6?.path}", i);
                    prescArray.add(prescription);
                  }
  ...
  Widget elevatedWidget(int index) {
    return ElevatedButton(
        ...
        onPressed: () async {
         ...
         // uploadPrescriptionToFirebase(image!);
        },
